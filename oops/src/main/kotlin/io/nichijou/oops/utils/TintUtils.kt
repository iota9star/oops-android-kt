...
import io.nichijou.utils.lighten
  ...
  fun tintSwitchDrawable(@NonNull context: Context, @NonNull from: Drawable, @ColorInt tint: Int, thumb: Boolean, compatSwitch: Boolean, useDarker: Boolean): Drawable? {
    ...
    if (thumb) {
      disabled = ContextCompat.getColor(context,
        if (useDarker) R.color.switch_thumb_disabled_material_dark else R.color.switch_thumb_disabled_material_light)
      normal = ContextCompat.getColor(context,
        if (useDarker) R.color.switch_thumb_normal_material_dark else R.color.switch_thumb_normal_material_light)
    } else {
      disabled = ContextCompat.getColor(context,
