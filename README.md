...
一个基于**LiveData** & **ViewModel** 的Android **Material** 主题引擎，由**Kotlin**实现。本主题引擎通过简单的方式实现各个**View**间无缝的色彩切换。整个过程除了当您更改了主题时才会触发重启**Activity**外，其余过程均无需重启**Activity**。支持``snackbar``和``toast``颜色更改。
